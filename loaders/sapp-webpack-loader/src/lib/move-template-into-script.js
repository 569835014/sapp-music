...
module.exports = function (__sAppParts__, config) {
    ...
    const templateLiteralAst = babel.parse(wrappedTemplateQuote(sAppTemplateString))
        .program
        .body[0]
        .expression;
    ...
    const sAppScriptAst = babel.parse(
        __sAppParts__.script.content
        // 注释掉  因为babel内部做了升级scoped babelrc
        // {
        //     extends: './.babelrc'
        // }
    );
    traverse(sAppScriptAst, {
        enter(path) {
